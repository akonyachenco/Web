...
public interface ProjectRepository extends JpaRepository<Project, Long> {
    ...
    @Query("SELECT p FROM Project p JOIN p.team e WHERE e.id = :id AND p.status.name = :status")
    List<Project> findAllByEmployeeIdAndStatusName(@Param("id") Long id, @Param("status") String status);

    Optional<Project> findBySprintsId(Long sprintsId);
