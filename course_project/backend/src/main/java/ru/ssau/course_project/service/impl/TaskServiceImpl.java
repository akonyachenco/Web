...
public class TaskServiceImpl implements TaskService {
    ...
    public TaskDto update(TaskDto dto) throws EntityNotFoundException {
        ...
        if (dto.getStatusName() != null && !dto.getStatusName().equals(task.getStatus().getName())) {
            Status status = statusRepository.findByName(dto.getStatusName())
                    .orElseThrow(() -> new EntityNotFoundException("Статус " + dto.getStatusName() + " не найден"));
    ...
    @Override
    public void changeStatus(Long id, String statusName) throws EntityNotFoundException {
        Status status = statusRepository.findByName(statusName).orElseThrow(
                () -> new EntityNotFoundException(String.format("Статус %s не найден", statusName))
        );

        Task task = taskRepository.findById(id).orElseThrow(
                () -> new EntityNotFoundException(String.format("Задача с id = %d не найден", id))
        );
